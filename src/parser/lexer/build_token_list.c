...
int get_token_value(char *input, int *i, t_token *token)
{
	...
	type = token->type;
	if(type == TOKEN_APPEND || type == TOKEN_HEREDOC || type == TOKEN_AND || type == TOKEN_OR)
	{
		token->value = ft_substr(input, *i, 2);
		*i += 2;
	}
	else if(type == TOKEN_REDIR_IN || type == TOKEN_REDIR_OUT || type == TOKEN_PIPE || type == TOKEN_LEFT_PAREN || type == TOKEN_RIGHT_PAREN )
	{
